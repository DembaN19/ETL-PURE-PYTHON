...
@task(name="Extract VAT Business Posting Group", retries=3, retry_delay_seconds=10)
def extract_vatbuspostinggroup():
    ...
    FROM 
    [company_name].[dbo].[table_business_central_prefix$VAT Business Posting Group$id_business_central]
    """
    ...
    output_path = DATA_LAKE_PATH / 'raw' / 'vatbuspostinggroup.parquet'
    df.to_parquet(str(output_path))
    logger.info(f"{len(df)} lignes extraites pour VAT Business Posting Group.")
    return df
