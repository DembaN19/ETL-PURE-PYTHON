...
@task(name="Extract Bank_Account_Posting_Group", retries=3, retry_delay_seconds=10)
def extract_bank_account_posting_group():
    logger = get_run_logger()
    
    query = """
    SELECT 
	Code
	,[G_L Bank Account No_] [GLBankAccountNo]
    ...
    FROM [company_name].[dbo].[table_business_central_prefix$Bank Account Posting Group$id_business_central]
    """
    ...
    output_path = DATA_LAKE_PATH / 'raw' / 'bank_account_posting_group.parquet'
    df.to_parquet(str(output_path))
    logger.info(f"{len(df)} lignes extraites pour Bank_Account_Posting_Group.")
    return df
