...
@task(name="Extract Detailed_Vendor_Ledger_Entry", retries=3, retry_delay_seconds=10)
def extract_detailed_vendor_ledger_entry():
    ...
    SELECT 
      ...
      ,[Applied Vend_ Ledger Entry No_] [AppliedVendLedgerEntryNo]
      ,[Unapplied] [Unapplied]
      ...
      ,[ACY Vendor Posting Group$df74057f-5b58-4d6f-8a52-989845f6320c] [ACYVendorPostingGroup]
      ,[ACY Employee Entry$df74057f-5b58-4d6f-8a52-989845f6320c] [ACYEmployeeEntry]
    FROM [company_name].[dbo].[table_business_central_prefix$Detailed Vendor Ledg_ Entry$id_business_central] A
	JOIN
		[company_name].[dbo].[table_business_central_prefix$Detailed Vendor Ledg_ Entry$id_business_central$ext]	B
    ...
    output_path = DATA_LAKE_PATH / 'raw' / 'detailed_vendor_ledger_entry.parquet'
    df.to_parquet(str(output_path))
    logger.info(f"{len(df)} lignes extraites pour Detailed_Vendor_Ledger_Entry.")
    return df
